...
class BubbleSort(QWidget):
    ...
    class element:
        def __init__(self,val,x,y):
            ...
            self.x=x
            self.y=y
            self.color=QColor(random.randint(100,240),random.randint(100,240),random.randint(100,240))
    ...
    def sort(self):
        ...
        for i in range(lenth-1, 0, -1):
            ...
            for j in range(i,lenth):
                self.array[j].color=self.array[i].color
        if self.array.__len__()>1:
            self.array[0].color=self.array[1].color
        self.tag=[-1,-1]
        self.bt1.setEnabled(True)
        self.state=False
